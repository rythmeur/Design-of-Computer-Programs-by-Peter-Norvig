...
z = ["ab", 'ds']
for r,s in z: # iterable можно разбивать на переменные
    print r,s

k = None
z = k or 28 # прикольно - если None то приравнивается другое значение
print z
...
def test_shuffler(shuffler, deck='abcd', n=10000):
    counts = defaultdict(int) # !!! интересно
    for _ in range(n):
        input = list(deck)
        shuffler(input)
        counts[''.join(input)] += 1
...
ah = "AS 2S 3S 4S 6C".split() # A high
sh = "2S 3S 4S 6C 7D".split() # 7 high
hands = [sf, fk, fh, tp, ah, tp, sf,sf]
print ' hand_rank', hand_rank(tp)
print ' hand_rank_table',  hand_rank_table(tp)
print ' allmax',  allmax(hands,hand_rank_table)
